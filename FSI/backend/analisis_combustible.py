...
def generar_reporte_pdf(df, mes, dependencia):
    ...
    try:
        ...
        # Configurar PDF
        pdf = FPDF()
        # Seguridad: algunas fuentes por defecto no soportan ciertos caracteres
        # Vamos a envolver pdf.cell para sanitizar textos y evitar errores con emojis/caracteres no latin1
        orig_cell = pdf.cell
        def _safe_cell(*a, **k):
            # Sanitize third positional arg (txt) or keyword 'txt'
            if len(a) >= 3:
                a = list(a)
                try:
                    a[2] = a[2].encode('latin-1', 'ignore').decode('latin-1')
                except Exception:
                    a[2] = str(a[2])
            elif 'txt' in k:
                try:
                    k['txt'] = k['txt'].encode('latin-1', 'ignore').decode('latin-1')
                except Exception:
                    k['txt'] = str(k['txt'])
            return orig_cell(*a, **k)
        pdf.cell = _safe_cell
        ...
        # Eliminar directorio temporal de forma recursiva (más robusto)
        import shutil
        temp_dir = 'temp_graficos'
        try:
            ...
            print(f"Warning: no se pudo eliminar directorio temporal {temp_dir}: {e}")
